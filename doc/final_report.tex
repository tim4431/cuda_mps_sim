...
\textbf{Phase-diagram structure.} Physically, this imbalance reflects the entanglement landscape: entropy peaks near the critical point ($J\approx g$), and the realized $\chi_a$ spans $2$--$6\times$ across the grid -- from $\chi_a\approx21$ in the paramagnetic corner to a saturated 64 at high $J$ -- which sets the per-pair SVD cost (Fig.~\ref{fig:phase}, Appendix).
