...
**Observables over the phase diagram.** Figure 7 shows $\langle\sigma^z_i\rangle$
and $\langle\sigma^x_i\sigma^x_{i+1}\rangle$ (nearest-neighbour correlator) collected
by the GPU sweep.  The transverse magnetization $\langle\sigma^x_i\rangle$ is
identically zero by parity symmetry — the initial all-$|0\rangle$ state has
$\langle\Pi\rangle=+1$ under the global flip $\Pi=\bigotimes\sigma^z_i$, and since
$\sigma^x_i\to-\sigma^x_i$ under $\Pi$ while the Hamiltonian commutes with $\Pi$,
the expectation value is pinned to zero throughout the evolution.  The ferromagnetic
corner (large $J$, small $g$) shows the largest NN correlations and smallest
$|\langle\sigma^z_i\rangle|$, consistent with the spin chain being dragged toward a
correlated ground state.

![Observable heatmaps over the 4×4 (J,g) grid.](figs/observables_heatmap.png)

*Figure 7. GPU-computed observables over the $(J,g)$ phase diagram ($L=20$,
$\chi_{\max}=64$, 20 Trotter steps at $dt=0.1$). Top-left: mid-chain entropy
(same data as Fig. 6, left). Top-right: mean $|\langle\sigma^z_i\rangle|$ (site
magnetization). Bottom-left: mean $|\langle\sigma^x_i\rangle|=0$ (zero by parity).
Bottom-right: nearest-neighbour $x$-correlator.*
...
2. **Analytical / exact-diagonalization oracle.** A full $2^L\times2^L$
   ...
   Trotter floor from the first step onward (Fig. 8).
...
*Figure 8. TEBD vs. exact diagonalization, per-step max error. Final fidelity
loss $4.3\times10^{-11}$.*
...
- **GPU-side observables.** The sweep now computes and reports full
  $\langle\sigma^z_i\rangle$, $\langle\sigma^x_i\rangle$, and two-point
  correlations $\langle\sigma^x_i\sigma^x_j\rangle$ for every $(J,g)$ pair.
  After each GPU evolution the device MPS is synced to host (a one-time copy
  per run), and `psi.expect()` / `psi.corr()` evaluate the observables on the
  host-side MPS; all $2L + L^2$ values are gathered to rank 0 via `MPI_Gatherv`
  alongside the existing timing and entropy fields.  Observable heatmaps over
  the $(J,g)$ phase diagram (mean $|\langle\sigma^z\rangle|$, mean
  $|\langle\sigma^x\rangle|$, nearest-neighbour correlator) are generated by
  `src/scripts/plot_observables.py`.
